...
expName = 'CvE_Comparison'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath=u'/Users/alexviegut/Box/0...AAV...0/PROJECT_OTHER/Comparison_vs_Estimation/Design/PsychoPy_ComparisonExperiment/CvE_Comparison.psyexp',
...
# Initialize components for Routine "GoodJob"
GoodJobClock = core.Clock()
GreatJobText = visual.TextStim(win=win, name='GreatJobText',
...
for thisInstruction in instructions:
    ...
    Instr1.setImage(instrImage)
    key_resp_instr = event.BuilderKeyResponse()
    # keep track of which components have finished
    instrComponents = [Instr1, key_resp_instr]
...
trials = data.TrialHandler(nReps=2, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('CvE_CompStimuli.xlsx'),
...
for thisTrial in trials:
    ...
    key_resp_stim = event.BuilderKeyResponse()
    # keep track of which components have finished
    trialComponents = [stimLeft, key_resp_stim]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if t >= 0.0 and key_resp_stim.status == NOT_STARTED:
            # keep track of start time/frame for later
            key_resp_stim.tStart = t
            ...
            event.clearEvents(eventType='keyboard')
        frameRemains = 0.0 + 0.5- win.monitorFramePeriod * 0.75  # most of one frame period left
        if key_resp_stim.status == STARTED and t >= frameRemains:
            key_resp_stim.status = STOPPED
        if key_resp_stim.status == STARTED:
            ...
            if "escape" in theseKeys:
                endExpNow = True
            if len(theseKeys) > 0:  # at least one key was pressed
                key_resp_stim.keys = theseKeys[-1]  # just the last key pressed
                key_resp_stim.rt = key_resp_stim.clock.getTime()
                # a response ends the routine
                continueRoutine = False
    ...
    for thisComponent in trialComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # check responses
    if key_resp_stim.keys in ['', [], None]:  # No response was made
        key_resp_stim.keys=None
    trials.addData('key_resp_stim.keys',key_resp_stim.keys)
    if key_resp_stim.keys != None:  # we had a response
        trials.addData('key_resp_stim.rt', key_resp_stim.rt)
    ...
    continueRoutine = True
    # update component parameters for each repeat
    decisionBlank = event.BuilderKeyResponse()
    # keep track of which components have finished
    blankComponents = [blankPage, decisionBlank]
    ...
    while continueRoutine:
        ...
        # *blankPage* updates
        if t >= 0.0 and blankPage.status == NOT_STARTED:
            # keep track of start time/frame for later
            blankPage.tStart = t
        ...
        if decisionBlank.status == STARTED:
            ...
            if "escape" in theseKeys:
                endExpNow = True
            if len(theseKeys) > 0:  # at least one key was pressed
                decisionBlank.keys = theseKeys[-1]  # just the last key pressed
                decisionBlank.rt = decisionBlank.clock.getTime()
                # a response ends the routine
                continueRoutine = False
...
# ------Prepare to start Routine "GoodJob"-------
t = 0
GoodJobClock.reset()  # clock
frameN = -1
continueRoutine = True
routineTimer.add(5.000000)
# update component parameters for each repeat
# keep track of which components have finished
GoodJobComponents = [GreatJobText]
for thisComponent in GoodJobComponents:
    if hasattr(thisComponent, 'status'):
        thisComponent.status = NOT_STARTED

# -------Start Routine "GoodJob"-------
while continueRoutine and routineTimer.getTime() > 0:
    # get current time
    t = GoodJobClock.getTime()
    frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
    # update/draw components on each frame
    
    # *GreatJobText* updates
    if t >= 0.0 and GreatJobText.status == NOT_STARTED:
        # keep track of start time/frame for later
        GreatJobText.tStart = t
        GreatJobText.frameNStart = frameN  # exact frame index
        GreatJobText.setAutoDraw(True)
    frameRemains = 0.0 + 5.0- win.monitorFramePeriod * 0.75  # most of one frame period left
    if GreatJobText.status == STARTED and t >= frameRemains:
        GreatJobText.setAutoDraw(False)
