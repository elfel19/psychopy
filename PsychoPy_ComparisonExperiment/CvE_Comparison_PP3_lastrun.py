...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/alexviegut/Box/0...AAV...0/PROJECT_OTHER/Comparison_vs_Estimation/Design/PsychoPy_ComparisonExperiment/CvE_Comparison_PP3_lastrun.py',
...
win = visual.Window(
    ...
    monitor='testMonitor', color='#C0C0C0', colorSpace='hex',
...
readyImage = visual.ImageStim(
    win=win,
    name='readyImage', 
    image='instrFiles/CircleComp_instr4.jpg', mask=None,
    ori=0, pos=(0, 0), size=(1, .5625),
    color=[1,1,1], colorSpace='rgb', opacity=1,
    flipHoriz=False, flipVert=False,
    texRes=128, interpolate=True, depth=0.0)
...
# set up handler to look after randomisation of conditions etc
pracTrials = data.TrialHandler(nReps=1, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('stims/CvE_CompStimuli_PRACTICE.xlsx'),
...
for thisPracTrial in pracTrials:
    ...
    pracTrials.addData('decisionStim.keys',decisionStim.keys)
    if decisionStim.keys != None:  # we had a response
        pracTrials.addData('decisionStim.rt', decisionStim.rt)
    ...
    pracTrials.addData('decisionBlank.keys',decisionBlank.keys)
    if decisionBlank.keys != None:  # we had a response
        pracTrials.addData('decisionBlank.rt', decisionBlank.rt)
...
# keep track of which components have finished
readyComponents = [readyImage, key_resp_ready]
...
while continueRoutine:
    ...
    # *readyImage* updates
    if t >= 0.0 and readyImage.status == NOT_STARTED:
        # keep track of start time/frame for later
        readyImage.tStart = t  # not accounting for scr refresh
        readyImage.frameNStart = frameN  # exact frame index
        win.timeOnFlip(readyImage, 'tStartRefresh')  # time at next scr refresh
        readyImage.setAutoDraw(True)
    
    # *key_resp_ready* updates
    if t >= 0.0 and key_resp_ready.status == NOT_STARTED:
        # keep track of start time/frame for later
        key_resp_ready.tStart = t  # not accounting for scr refresh
        ...
        # keyboard checking is just starting
        key_resp_ready.clearEvents(eventType='keyboard')
